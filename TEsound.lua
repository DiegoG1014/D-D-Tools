...
function TEsound.play(sound, tags, volume, pitch, func)
	if type(sound) == "table" then
		if (#sound > 0) then
      log.record("TEsound", "The list of sounds must have at least one sound.", true)
      return
    end
		sound = sound[math.random(#sound)]
	end
	if not (type(sound) == "string" or (type(sound) == "userdata" and type(sound) == "SoundData")) then
    
		log.record("TEsound", "You must specify a sound - a filepath as a string, a SoundData, or a table of them. Not a Source!", true)
    return
  ...
  log.record("TEsound", "Playing: '"..tostring(s).."' with volume of: "..(volume or 1).."; And pitch of: "..(pitch or 1))
	s[1]:play()
...
function TEsound.playLooping(sound, tags, volume, pitch, n)
...
function TEsound.pause(channel)
  log.record("TEsound", "Pausing: "..tostring(channel))
  ...
  else log.record("TEsound", "Attempted to pause: "..channel..", but not a number or string.", true) end
end

--- Resumes a channel or tag from a pause.
-- @param channel See TEsound.volume
function TEsound.resume(channel)
  log.record("TEsound", "Resuming: "..tostring(channel))
...
function TEsound.stop(channel, finish)
  if finish then log.record("TEsound", "Stopping: '"..tostring(channel).."' but allowing it to finish.") else log.record("TEsound", "Stopping: '"..tostring(channel).."'") end
