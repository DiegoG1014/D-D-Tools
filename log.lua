log = {records = {}, fullrecords = {}, strict = false, sublog = false, override_errhand = false,logsaved = false}
do
...
if conf.debug.strict then log.strict = true else log.strict = false end
...
function log.record(source, data, iserror, gravity, catchCall)
  ...
  if iserror then
    ...
    lua_assert(gravity <= 1,("[%s](%s)"):format(source,data))
    lua_assert(not log.strict, ("{%d:%d:%d}[%s](%s) ||ERROR"):format(os.date("%H"),os.date("%M"),os.date("%S"),source,data))
...
function log.subrecord(source,data)
  if log.strict and log.sublog then
    local str = ("///{%d:%d:%d}[%s](%s)"):format(os.date("%H"),os.date("%M"),os.date("%S"),source,data)
...
function log.savelog(crash)
  ...
  if crash then
    ...
    saving.write(log.title..".log",string.format("%s%s%s",header,body,footer),mode,"log",false,"logs",true)
    
  end
  
end

if log.override_errhand then
